    ...
    private static Pair<AGate, List<String>> parseAGate(XMLNode node) {
        ...
        switch (node.getName()) {
            ...
            case "false":
                ...
                break;
            case "clock":
                if (!node.getAttributes().containsKey("duty")) {
                    gate = new Clock(name, Boolean.parseBoolean(node.getAttributes().get("init")), Double.parseDouble(node.getAttributes().get("freq")));
                } else {
                    gate = new Clock(name, Boolean.parseBoolean(node.getAttributes().get("init")), Double.parseDouble(node.getAttributes().get("freq")), Double.parseDouble(node.getAttributes().get("duty")));
                }
                break;
            case "not":
                gate = new Not(name);
